...
use vulkano::{ buffer::{BufferContents, BufferCreateInfo, BufferUsage}, image::{Image, ImageAspects, ImageSubresourceLayers, ImageSubresourceRange}, memory::allocator::{AllocationCreateInfo, DeviceLayout, MemoryTypeFilter}, pipeline::{ComputePipeline, Pipeline, PipelineShaderStageCreateInfo, compute::ComputePipelineCreateInfo}};
use vulkano_taskgraph::{Id, Task, TaskContext, command_buffer::{BufferImageCopy, CopyBufferToImageInfo, DependencyInfo, ImageMemoryBarrier}, resource::HostAccessType};
...
impl Pixel {
    fn new_random(rng: &mut ThreadRng) -> Self {
        Self { position: [rng.random(), rng.random(), rng.random(), 255] }
    }
}
...
impl ComputeTask {
    pub fn new(app: &App, image: Id<Image>) -> Self {
        ...
        let pixels: Vec<Pixel> = (0..1024*1024).into_iter()
            .map(|_| Pixel::new_random(&mut rng))
            .collect();

        let upload_buffer = app.resources.create_buffer(
            &BufferCreateInfo {
